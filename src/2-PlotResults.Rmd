...
# 2000 to 2100_8.5  
  ...
  legend(40, 12, c("2000", "2100"), fill=c(adjustcolor("goldenrod", 0.8), 
         adjustcolor("darkmagenta", 0.5)), bty="n", adj=0, cex=1.5)

### PH ######
# 1800 to 2000  PH
  image(xb, yb, z_1800b, col=c("white", adjustcolor("dodgerblue4",0.8)),
        xlab="Temperature (C)",
        ylab="pH",
        bty="l",
        xlim=c(-5, 60),
  ...
  image(xb, yb, z_2000b, add=TRUE, col=c(rgb(0,0,0,0), adjustcolor("goldenrod", 0.8)))
  text(0, 11, "C", cex=2)
  #text(20, 7.5, "1800 disappearing in 2000", col="dodgerblue4")  
  #lines(c(20,25), c(7, 4.6), lwd=3, col="dodgerblue4")
  #lines(c(20,5.3), c(7, 4.1), lwd=3, col="dodgerblue4")
...
# 2000 to 2100_8.5  PH
  image(xb, yb, z_2000b, col=c(rgb(0,0,0,0), adjustcolor("goldenrod", 0.8)),
        xlab="Temperature (C)",
        ylab="",
        bty="l",
        xlim=c(-5, 60),
  ...
  image(xb, yb, z_2100_8.5b, add=TRUE, col=c(rgb(0,0,0,0), adjustcolor("darkmagenta", 0.5)))
  #text(20, 6.5, "2000 disappearing in\n2100 RCP 8.5", col="goldenrod4")  
  text(0, 11, "D", cex=2)
  #lines(c(20,20), c(5.5, 3), lwd=3, col="goldenrod4")
  #lines(c(20,10), c(5.5, 2), lwd=3, col="goldenrod4")
...
## Other plotting not used in manuscript

```{r, try to plot reason why non-inutitive result}
head(dat1norm_2000)
par(mfrow=c(1,1))
plot(dat1norm_2000$Lon, dat1norm_2000$Lat)

# Choose a site in the N. Pacific ~45 degrees N
# with a low degree of novelty but a high
# degree of disappearance
head(dat_Nov)
my_spot <- dat_Nov %>% filter(long==199.5 & lat==46.5) %>% 
  select(No, lat, long, NN.sigma_today_2100_8.5, NN.station_today_2100_8.5, lat_today_2100_8.5, long_today_2100_8.5, NN.sigma_2100_8.5_today, NN.station_2100_8.5_today, lat_2100_8.5_today, long_2100_8.5_today)

dat_Nov %>% filter(long==199.5 & lat==0.5) %>% 
  select(No, lat, long, NN.sigma_today_2100_8.5, NN.station_today_2100_8.5, lat_today_2100_8.5, long_today_2100_8.5, NN.sigma_2100_8.5_today, NN.station_2100_8.5_today, lat_2100_8.5_today, long_2100_8.5_today)

my_spot
# NN.sigma_today_2100_8.5 today baseline, degree of novelty for this location
# NN.station_today_2100_8.5 today baseline, location of nearest neighbor in today's data for the future climate of this location
# lat_today_2100_8.5, long_today_2100_8.5

# NN.sigma_2100_8.5_today future baseline, degree of disappearance for today's climate
# NN.station_2100_8.5_today, lat_2100_8.5_today, long_2100_8.5_today future baseline, location of nearest neighbor in future data for this location

(today_myspot <- dat1 %>% filter(No==my_spot$No & Year>1950 & Month>=6 & Month <=8) %>% summarise_each(mean))
#   No   Lon  Lat Year Month      SST     Arag       pH
# 1 18952 199.5 46.5 1985     7 10.77055 2.070625 8.082542

(future_myspot <- dat8.5 %>% filter(No==my_spot$No &  Month>=6 & Month <=8) %>% summarise_each(mean))
(novelty_nn <- dat1 %>% filter(No==my_spot$NN.station_today_2100_8.5& Year>1950 & Month>=6 & Month <=8) %>% summarise_each(mean))
  # This should be close to future_myspot

(today_myspot <- dat1norm_2000 %>% filter(No==my_spot$No))
(disappear_nn <- dat1norm_8.5 %>% filter(No==my_spot$NN.station_2100_8.5_today))
  # This (should) be close to today_myspot
