...
C.id <- C$No
 ...
 (PCs <- max(c(which(unlist(summary(PCA)[1])>trunc.SDs),1)))
    ...
    points(x[cond1], y[cond1], col=NA, bg=adjustcolor("darkgrey", 0.5), pch=21)
    points(x[cond2], y[cond2], col=adjustcolor("grey",0.5), bg=adjustcolor("lightgrey", 0.5), pch=21)
...
pdf("../figures/EXAMP_legend.pdf", width=3, height = 3)
  plot(0,0, xlim=c(0,10), ylim=c(0,10))
  legend(0,5, c("Focal station", "NN"), col=c("blue", "black"), pch=c(19,23), pt.bg=c("black", "green"), bty="n")
dev.off()
