...
# TO DO
## COMBINE LAT vs. variable PLOTs

variables in rows, years in columns

* Lat vs. SST (g0, g2, g3)
* Lat vs. Arag (a0, a2, a3)
* Lat vs pH (p0, p2, p3)
* add legend
```{r}
...
p1N <- p0N +  geom_hex(data= dat_1800[plotind_1800,], aes(SST, pH), fill=c1800, alpha=0.5) +
  ...
  geom_segment(aes(x=15, y = 8.65, xend=23, yend=8.25), color="grey30") +
  annotate(geom="text", x=15, y=7.6, label="Novel in 2000", adj=0, color=adjustcolor(c2000,0.5)) +
  geom_segment(aes(x=15, y = 7.63, xend=7, yend=8.0), color=adjustcolor(c2000,0.5)) +
  geom_segment(aes(x=15, y = 7.63, xend=25, yend=7.95), color=adjustcolor(c2000,0.5)) 
#p1N
