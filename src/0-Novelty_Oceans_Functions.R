...
Plot_interp<-function(data,column,title){ #Data is expected to be data.table format, column should be in the format 'data$column', title is a character string for the figure legend.
  ...
  MakeRast<-function(data){
    while(boolFALSE==F){
      tryCatch({
        ...
        data.idw <- idw(sigma ~ 1, data, newdata = gr, idp = 2)
  ...
  tm_shape(r) + 
    tm_raster(n = 12, palette = 'Reds', # n = 10 may be better
              title = title) + 
