...
# This code uses the seasonal climate normals and seasonal ICV (summer vs. winter) for the novelty calculation
...
#-------------------------------- 
  ### ICV matrix
  head(ICV)  
  (whichcols <- which(names(ICV) %in% c("SST", "Arag", "pH")))
  ICV_summer <- ICV %>% filter((Lat >=0 & Month %in% c(6,7,8)) | 
  ...
  ICV_winter <- ICV %>% filter((Lat <0 & Month %in% c(6,7,8)) | 
                                      (Lat >0 & Month %in% c(12,1,2))) %>% 
                select(SST, Arag, pH) 
...
NN.dist <- seq(8, 20, 0.0001)
NN.chi <- pchi(NN.dist,PCs)
NN.sigma <- qchi(NN.chi,1) 
plot(NN.dist, NN.sigma)

log_NN.chi <- log10(NN.chi)
NN.sigma <- qchi(10^log_NN.chi,1) 
plot(NN.dist, NN.sigma)

NN.dist <- seq(8, 20, 0.0001)
NN.chi <- pchi(NN.dist,PCs, rel.tol=.Machine$double.eps^0.8)
NN.chi[NN.chi>=(1-1e-16)]<-1-1e-16
NN.sigma <- qchi(NN.chi,1) 
plot(NN.dist, NN.sigma)
min(NN.sigma[NN.dist>9])
#if(NN.dist>9 & NN.sigma>8.12){NN.sigma <- qchi(1-1e-16,1) }
NN.sigma[NN.dist>9 & NN.sigma>8.12] <- qchi(1-1e-16,1)
plot(NN.dist, NN.sigma)
